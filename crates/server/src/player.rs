...
use networking::{
    replication::NetIdGen,
    server::{ClientConnectMessage, OwnedByClient},
};
use shared::{
    components::*,
    consts::CLIENT_TICK_RATE,
    functions::simulate_player,
    messages::{AssignLocalPlayerMessage, ServerTickMessage},
    resources::*,
    structs::{AssignLocalPlayer, InputPayload},
};
...
pub fn spawn_players_system(
    ...
    for message in connect_message.read() {
        ...
        let player_entity = commands
            ...
            .insert(Player {
                client_id: message.client_id as u64,
            })
...
pub fn process_inputs_system(
    ...
    for _tick in server_tick_message.read() {
        for (client_id, input_state) in input_state_map.0.iter_mut() {
            ...
            let Some(player_entity) = server_lobby.players.get(client_id) else {
                continue;
            };
            ...
            input_state
                .buffer
                .retain(|p| p.seq > input_state.last_processed_input.seq);
        }
    }
}
